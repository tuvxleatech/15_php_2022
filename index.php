                ...
                <?php foreach ($result as $each) : ?>
                    <div class="list-item">
                        ...
                        <a class="image-link" href="detail.php?id=<?php echo $each['id'] ?>">
                            <img class="list-item-image" src="<?php echo "./" . substr($each['image'], 5) ?>" alt="" />
                        </a>
                        <span class="old-price">Giá cũ: <del><?php echo $each['price'] * 1.8 ?>vnd</del></span>
                        <span class="new-price">Giá bán: <?php echo $each['price'] ?> vnd</span>
                        <span class="manufacturer">Nhà sản xuất:
                            <?php
                            $id_manufacturer = $each['id_manufacturer'];
                            $sql = "select * from manufacturers where id = '$id_manufacturer'";
                            $result = mysqli_query($connect, $sql);
                        ...
                        <?php if (!empty($_SESSION['id_customer'])) { ?>
                            <button class="btn-add-to-cart btn btn-primary prettyprint runnable prettyprinted" data-id="<?php echo $each['id'] ?>">
                                Thêm vào giỏ hàng
                            </button>
                        <?php } else { ?>
                            <a class="btn-add-to-cart btn btn-primary" href="login.php">
        ...
        <div>
            ...
            <div class="list">
                <div class="list-item">
                    ...
                    <span class="manufacturer">Nhà sản xuất:
                        <?php
                        $id_manufacturer = $each_product_best_selling['id_manufacturer'];
                        $sql = "select * from manufacturers where id = '$id_manufacturer'";
                        $result = mysqli_query($connect, $sql);
                        $manufacturer = mysqli_fetch_array($result);
                        echo $manufacturer['name'];
                    ...
                    <?php if (!empty($_SESSION['id_customer'])) { ?>
                        <button class="btn-add-to-cart btn btn-primary prettyprint runnable prettyprinted" data-id="<?php echo $each_product_best_selling['id'] ?>">
                            Thêm vào giỏ hàng
                        </button>
                    <?php } else { ?>
                        <a class="btn-add-to-cart btn btn-primary" href="login.php">
